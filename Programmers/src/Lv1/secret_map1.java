...
public class secret_map1 {
	
	public static String[] solution(int n, int[] arr1, int[] arr2) {
        String[] answer = new String[n];
        
		StringBuffer deciFormat = new StringBuffer();
		for (int inx=0; inx<n; inx++ ) deciFormat.append("0");
		
		DecimalFormat df = new DecimalFormat(deciFormat.toString());
		for(int i=0; i<n; i++) {
			String res = Long.toBinaryString(arr1[i] | arr2[i]);
			res = df.format(Long.parseLong(res));
			answer[i] = res;
		}
		
		for(int j=0; j<n; j++) {
			answer[j] = answer[j].replace("0", " ").replace("1", "#");
		}
